...
func handleBridgeMessage(db Database, event string, data any) error {
	...
	if event == "create_message" {
		...
		if _, err := db.getMessage(base.EventID); err == nil {
			Log.Trace().Str("channel", base.ChannelID).Str("message", base.EventID).Msg("Skipping duplicate message")
			return nil
		}
		...
		if err == nil {
			...
			if bridgeMsg.ID != base.EventID && event == "edit_message" {
				Log.Trace().Str("event_id", base.EventID).Msg("Message is not the original, skipping bridge edit handling")
				return nil
			}
		}
	}
