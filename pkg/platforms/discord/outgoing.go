...
func ifNotNil[T any](thing *T) T { //nolint:ireturn
	if thing != nil {
		return *thing
	}
...
func lightningToDiscordSendable(
	...
	toSend := []discordSendable{{
		...
		AvatarURL: ifNotNil(msg.Author).ProfilePicture,
		Username:  ifNotNil(msg.Author).Nickname,
	}}
	...
	if len(toSend[0].Content) > 2000 {
		...
		chunks := make([]string, 0, int(math.Ceil(float64(len(leftover))/2000)))

		for i := 0; i < len(leftover); i += 2000 {
		...
		for _, chunk := range chunks {
			toSend = append(toSend, discordSendable{
				...
				MessageSend: discordgo.MessageSend{Content: chunk},
			})
		}
	}
