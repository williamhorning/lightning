...
func (p *guildedPlugin) SetupChannel(channel string) (any, error) {
	...
	if resp.StatusCode != 200 && resp.StatusCode != 201 {
		return nil, lightning.LogError(errors.New("Failed to get channel: "+strconv.Itoa(resp.StatusCode)), "Failed to get channel for setup", map[string]any{"status": resp.StatusCode, "body": string(bodyBytes)}, nil)
	}
	...
	if resp.StatusCode != 200 && resp.StatusCode != 201 {
		...
		return nil, lightning.LogError(errors.New("Failed to create webhook: "+strconv.Itoa(resp.StatusCode)), "Failed to create webhook for channel", extra, nil)
	}
	...
	if webhookData.Webhook.Token == nil {
		return nil, lightning.LogError(errors.New("webhook token is nil"), "Failed to create webhook for channel", map[string]any{"channelID": channel, "webhook": webhookData}, nil)
	}
