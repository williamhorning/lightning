import type { Channel, Embed, Member, Message, User } from '@jersey/revolt-api-types';
...
async function get_author(api: Client, author_id: string, channel_id: string): Promise<message_author> {
    try {
        const channel = await api.request('get', `/channels/${channel_id}`, undefined) as Channel;

        const author = await api.request('get', `/users/${author_id}`, undefined) as User;
        ...
        const author_data = {
            ...
            color: '#FF4654',
            profile: author.avatar ? `https://autumn.revolt.chat/avatars/${author.avatar._id}` : undefined,
        }
        ...
        if (channel.channel_type !== 'TextChannel') {
            ...
            try {
                const member = await api.request('get', `/servers/${channel.server}/members/${author_id}`, undefined) as Member;

                return {
                    ...author_data,
                    username: member.nickname ?? author_data.username,
                    profile: member.avatar ? `https://autumn.revolt.chat/avatars/${member.avatar._id}` : author_data.profile,
                }
            } catch {
                return author_data
            }
