...
export async function execute_text_command(msg: message, lightning: lightning) {
	...
	const [command, ...rest] = msg.content.replace(lightning.config.prefix, '')
		.split(' ');
	...
	return await run_command({
		...
		reply: async (message: message) => {
			await lightning.plugins.get(msg.plugin)?.send_message({
				...message,
...
export async function run_command(
	...
	let command = lightning.commands.get(opts.command) ??
		lightning.commands.get('help')!;
	...
	try {
		resp = await command.execute({
			prefix: lightning.config.prefix,
			...opts,
			args: opts.args as Record<string, string>,
			bridge_data: lightning.data,
			plugins: lightning.plugins,
		});
	} catch (e) {
	...
	try {
		if (typeof resp === 'string') {
			await opts.reply(create_message(resp));
		} else await opts.reply(resp.msg);
	} catch (e) {
