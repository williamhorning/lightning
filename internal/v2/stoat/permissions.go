...
func (session *Session) calculateUserPermissions(self *User, channel *Channel) Permission {
	...
	recipient := Get(session, "/users/"+userID, userID, &session.UserCache)
	if recipient == nil {
		return PermissionSet3
	}
...
func (session *Session) calculateServerPermissions(channel *Channel, user *User) Permission {
	server := Get(session, "/servers/"+*channel.Server, *channel.Server, &session.ServerCache)
	if server == nil {
		return 0
	}
	...
	member := Get(session, "/servers/"+server.ID+"/members/"+user.ID, server.ID+"-"+user.ID, &session.MemberCache)
	if member == nil {
		return 0
	}
